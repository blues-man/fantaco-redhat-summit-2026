...
# Pin the gateway image rather than accepting the operator default. An unset
# spec.image means each namespace gets whatever the operator hands out the day
# it is created, which split a 50-user fleet across two OpenClaw versions.
...
SM_EOF
  ...
  if [[ -n "${LANGFUSE_PUBLIC_KEY:-}" && -n "${LANGFUSE_SECRET_KEY:-}" && -d "$PLUGINS_DIR/langfuse-tracer" ]]; then
    ...
    if [[ -n "$POD" ]]; then
      ...
      # Verify rather than trust the exit codes. `oc cp` has been seen to report
      # success while leaving a file absent, and a half-copied plugin directory
      # is worse than none: the repatch enables the plugin and the gateway then
      # refuses to boot. If it is not complete, remove it so the repatch's own
      # check skips it cleanly.
      if [[ "$PLUGIN_OK" == "true" ]]; then
        if oc exec "$POD" -n "$NS" -c gateway -- \
             sh -c "test -s '${PLUGINS_DEST}/langfuse-tracer/index.js' && test -s '${PLUGINS_DEST}/langfuse-tracer/openclaw.plugin.json'" 2>/dev/null; then
          echo -e "  ${GREEN}✓${RESET} $NS: langfuse-tracer files copied"
        else
          echo -e "  ${YELLOW}⚠${RESET} $NS: langfuse-tracer incomplete after copy — removing"
          oc exec "$POD" -n "$NS" -c gateway -- rm -rf "${PLUGINS_DEST}/langfuse-tracer" 2>/dev/null || true
        fi
      else
        oc exec "$POD" -n "$NS" -c gateway -- rm -rf "${PLUGINS_DEST}/langfuse-tracer" 2>/dev/null || true
      fi
  ...
  # workspace/main/AGENTS.md — patching only workspace/AGENTS.md (as this did)
  # means the append never reaches the running agent. The shared copy still
  # matters so agents created later inherit it.
  # Guard per '## ' section rather than on one sentinel, or a new section added
  # to the template never lands on a namespace patched by an earlier version.
  oc exec "$POD" -n "$NS" -c gateway -- node -e "
    const fs = require('fs');
    const sections = ${AGENTS_APPEND_JSON}.split(/\n(?=## )/).filter(s => s.trim());
